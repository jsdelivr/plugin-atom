...
export async function getFiles (pkg) {
	let response;

	try {
		response = await got(`${pkg.name}@${pkg.selectedVersion}/flat`, { baseUrl: jsDelivrEndpoint, headers: { 'user-agent': userAgent } });
	} catch (e) {
